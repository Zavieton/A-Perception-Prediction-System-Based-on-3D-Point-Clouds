...
def draw_scenes(points, ref_boxes=None, ref_id=None):
    ...
    if ref_boxes is not None and len(ref_boxes) > 0:
        ...
        # print(ref_corners3d[0].shape)
        # print("aaaa", ref_corners3d)
        for i in range(len(ref_boxes)):
            id = ref_id[i]
            fig = draw_corners3d(ref_corners3d[i], fig=fig, color=(np.abs(np.sin(id)), np.abs(np.cos(id)), np.exp(id)%1 ), cls=id, max_num=100)
    ...
    for n in range(num):
        ...
        if cls is not None:
            if isinstance(cls, np.ndarray):
                mlab.text3d(b[6, 0], b[6, 1], b[6, 2], '%.2f' % cls, scale=(0.3, 0.3, 0.3), color=color, figure=fig)
            else:
                mlab.text3d(b[6, 0], b[6, 1], b[6, 2], '%s' % cls, scale=(0.3, 0.3, 0.3), color=color, figure=fig)
